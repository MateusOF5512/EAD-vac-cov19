...
def pie_ind_popvac( df_selection ):
    ...
    df_filter = df_selection.drop(['meses_aplicacao', 'paciente_idade',
                                   "lat", "long",
                              'Feminino', 'Masculino',
                              'Covishield', 'Pfizer', 'Coronavac', 'Janssen', 'AstraZeneca',
...
# 1.3 - Análise das Vacinas Aplicadas - APRESENTAÇÂO DOS DADOS ---------------------------------------------
    ...
    y_Pfizer = [df['Pfizer'][0], df['Pfizer'][1], df['Pfizer'][4], df['Pfizer'][3]]
    y_AstraZeneca = [df['AstraZeneca'][0], df['AstraZeneca'][1], df['AstraZeneca'][4], df['AstraZeneca'][3]]
    y_Coronavac = [df['Coronavac'][0], df['Coronavac'][1], df['Coronavac'][4], df['Coronavac'][3]]
    y_Janssen = [df['Janssen'][0], df['Janssen'][1], df['Janssen'][4], df['Janssen'][3]]
...
def map_density( df_selection ):
    st.header("3.0 - Mapa da Vacinação")

    row_spacer1, row_1, row_spacer2 = st.columns((.2, 7.1, .2))
    df_filter = df_selection.drop([
        "paciente_idade", "meses_aplicacao", ], axis=1)
    df_new = df_filter.groupby(["estabelecimento_municipio_nome", 'estalecimento_nofantasia',
...
# --------------------------------------------  Map Deensity
    ...
    coordenadas = []
    for lat, long in zip(df_filter["lat"], df_selection["long"]):
...
#--------------------------------------------------------------------------
    ...
    df_mapa2 = df_selection.groupby(["estabelecimento_municipio_nome", 'estalecimento_nofantasia',
